...
export class AuthenticationService {
    ...
    public async login(loginData: ILogin) : Promise<ILoginResponse> {
        ...
        const arePasswordsEqual = this.compareLoginPasswords(password,userFound.password);
        if(!arePasswordsEqual) throw new ConflictException('The password is incorrect');
    ...
    private compareLoginPasswords(loginPassword: string, userFoundPassword: string){
        let arePasswordsEqual = bcrypt.compareSync(loginPassword, userFoundPassword);
