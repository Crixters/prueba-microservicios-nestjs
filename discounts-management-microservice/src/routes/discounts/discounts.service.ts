...
export class DiscountsService {
    ...
    public async getDiscountByCode(discountCode: string) : Promise<IDiscount> {
        const discountFound = await this.discountRepository.findOne({where: {code: discountCode}});
        if(!discountFound) throw new NotFoundException("Couldn't find discount with the code: "+discountCode);
        return discountFound;
    }

    public async registerDiscount(discountData: IRegisterDiscount) : Promise<IDiscount> {
        const discountCode = this.generateDiscountCode();
        const newDiscountInstance = this.discountRepository.create({...discountData, code: discountCode});
        return await this.discountRepository.save(newDiscountInstance);
    }

    public async validateDiscount(discountData: IValidateDiscount) : Promise<IValidateDiscountResponse> {
        const { discountCode, houseId, userId } = discountData;
        const discountFound = await this.discountRepository.findOne({where: { houseId, userId, code: discountCode }});
        if(!discountFound) return { isValidDiscount: false }
        return { isValidDiscount: true };
    }

    public async deleteDiscountRegister(discountRegisterId: string) : Promise< IDeleteDiscountRegisterResponse> {
        const discountFound = await this.discountRepository.findOne({where:{id: discountRegisterId}});
        if(!discountFound) throw new NotFoundException("No discount register found with the id: "+discountFound);
